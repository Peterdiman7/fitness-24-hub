<template>
    <div class="recipes-list container">
        <RouterLink to="/nutrition" class="back-link">← Back to Nutrition Categories</RouterLink>
...
const fetchData = async () => {
    ...
    try {
        const catRes = await fetch(`http://localhost:9102/nutrition/categories/${route.params.id}`, {
            credentials: "include"
        })
        if (catRes.ok) category.value = await catRes.json()

        const recipesRes = await fetch(`http://localhost:9102/nutrition/categories/${route.params.id}/recipes`, {
            credentials: "include"
        })
